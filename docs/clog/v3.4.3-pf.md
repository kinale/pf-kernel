...
Here is new pf-kernel release that brings Linux updated to version 3.4.4. You may grab patch [here](http://pf.natalenko.name/sources/3.4/patch-3.4.3-pf.bz2). Remember to apply it against **bare** 3.4 tree ( _with no stable patches from gkh, they are already in pf-kernel — just small reminder from me to those who write me sometimes_ ).  
