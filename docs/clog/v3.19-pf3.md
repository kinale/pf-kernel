Hello, mates. I guess it's time to back to usual 1-pf-per-2-stable pf-kernel release cycle, so I'm proud to introduce 3.19-pf3. The patch is available [here](https://pf.natalenko.name/sources/3.19/patch-3.19-pf3.xz).  
  
TL;DR: resync against stable tree, Alfred's Chen BFS port (updating BFS to v461 by Con + preempt_schedule_common() by Frederic Weisbecker and kernelOfTruth), graysky's patch update (Silvermont support introduced). That's all.  
