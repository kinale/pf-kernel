...
Another pf-kernel release is here. It incorporates latest 3.11.6 stable kernel patch from GKH and small TCP fix merged to fix [this issue](https://lkml.org/lkml/2013/8/21/156).  
  
In general, no prominent changes occurred, it's just stable release, so go and build it right now using [this patch](https://pf.natalenko.name/sources/3.11/patch-3.11.4-pf.bz2).  
