Buonanotte, amici miei. I'm pleased to introduce another pf-kernel release — v4.2-pf2. And here [the patch](https://pf.natalenko.name/sources/4.2/patch-4.2-pf2.xz) goes.  
  
This release incorporates latest stable fixes (up to 4.2.3 incl.) as well as updated BFS. I've decided to test Alfred's -vrq branch, and it seems to work flawlessly on all my 3 machines, so I encourage you to do the same. In case of any issues, please, show your .config file first.  
  
Unfortunately, TOI is still not fixed for Btrfs, although at least we've prepared special VM for testing it.  
